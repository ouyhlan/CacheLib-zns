...
class ClockPolicy {
 ...
 public:
  ...
  // the lock was held by ClockSegmentIndex
  // looks ugly, but indeed we need to write like this
  void update(uint16_t offset, LogicalPageOffset new_logical_page_offset) {
    auto* curr = findEntry(offset);
    if (!curr->ghost()) {
      collect_valid_fn_(new_logical_page_offset, curr->size());
    }
  }
  ...
  void setHotDataThreshold(uint64_t hot_data_threshold) {
    hot_data_threshold_ = hot_data_threshold;
