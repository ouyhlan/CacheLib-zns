...
ClockSegmentIndex::ClockSegmentIndex(uint64_t num_buckets_per_partition,
    ...
    : num_mutexes_(num_buckets_per_partition / 10 + 1),
      ...
      admission_policy_(
          num_buckets_, bf_num_hashes, bf_bucket_bytes, setid_fn_),
      eviction_policy_(
          allocation_size,
          hot_data_threshold,
          admission_policy_,
...
void ClockSegmentIndex::allocate() {
  XDCHECK_LE((num_allocations_ + 1) * allocation_size_, null_entry_offset_);
...
void ClockSegmentIndex::readmit(HashedKey hk,
                                size_t size,
                                LogicalPageOffset logical_page_offset) {
  const auto bucket_id = getBucketId(hk);
  uint32_t tag = createTag(hk.key());
...
void ClockSegmentIndex::updateEntry(uint16_t curr_offset,
                                    ...
                                    uint32_t tag) {
  curr_entry->setEntry(logical_page_offset, tag);
  eviction_policy_.update(curr_offset, logical_page_offset);
}
