...
class CuckooSet {
 public:
  ...
  explicit CuckooSet(Config&& config,
                     ZoneManager& zns_mgr,
  ...
  void insert(std::vector<ObjectInfo> object_vec, LogReadmitCallback readmit);
 ...
 private:
  ...
  uint32_t num_zones_;
  SetAsyncStorageManager async_mgr_;

  std::unique_ptr<BloomFilter> bloom_filter_;

  CuckooHashMap index_;

  TrackSetInsertFn track_fn_;

  struct ValidConfigTag {};
  CuckooSet(Config&& config,
            ZoneManager& zns_mgr,
            JobScheduler& scheduler,
            TrackSetInsertFn track_fn,
            ValidConfigTag);
  ...
  mutable AtomicCounter bf_false_positive_count_;
};
