...
std::vector<uint32_t> ZoneManager::allocate(uint32_t num_allocate_zones,
  ...
  if (zone_type == ZoneNandType::SLC) {
    device_.changeZonesIntoSLC(zone_need_to_change);
  } else if (zone_type == ZoneNandType::QLC) {
    device_.changeZonesIntoQLC(zone_need_to_change);
  }
...
Buffer ZoneManager::readPage(FlashByteAddressT flash_byte_address) {
  ...
  device_.read(flash_byte_address, buffer.size(), buffer.data());
...
Task<uint32_t> ZoneManager::getFreeZoneFromSet() {
  XDCHECK(set_mgr_ != nullptr);

  uint32_t free_zone_id = co_await set_mgr_->getFreeZoneId();
  co_return free_zone_id;
}

Task<uint32_t> ZoneManager::getFreeZoneFromAdaptive() {
  XDCHECK(adaptive_mgr_ != nullptr);

  uint32_t free_zone_id = co_await adaptive_mgr_->getCleanZoneId();
  co_return free_zone_id;
}
...
void ZoneManager::returnZoneToSet(uint32_t zone_id) {
  ...
  set_mgr_->addFreeZone(zone_id).detach();
}
