...
Zone::Zone(ZNSDevice& device,
    ...
    : device_(device),
      ...
      zone_type_(zone_size_byte_ == capacity_byte_ ? ZoneNandType::QLC
                                                   : ZoneNandType::SLC),
      segment_arr_(zone_size_byte_ / segment_size_byte_),
      page_arr_(zone_size_byte_ / page_size_byte_) {}

auto Zone::appendPage(SetIdT set_id,
                      Buffer& buffer) -> std::tuple<bool, FlashByteAddressT> {
  XDCHECK(buffer.size() == page_size_byte_);

  uint64_t next_entry_byte_offset_ =
      last_entry_end_byte_offset_.add_fetch(page_size_byte_);
