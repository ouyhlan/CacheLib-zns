...
class ZoneHashProtoImpl final : public ZoneHashProto {
 public:
  ...
  void setIoScheduler(std::string char_device_path) override {
    config_.char_device_path = char_device_path;
  }
  ...
  void setAdaptive(uint32_t max_num_zones,
    ...
    config_.shadow_config.adaptive_zone_capacity =
        config_.adaptive_config.zone_capacity_byte;
  }
  ...
  void setLayout(uint32_t max_num_zones,
    ...
    if (config_.set_config.zone_nand_type == ZoneNandType::QLC) {
      ...
      config_.shadow_config.set_zone_capacity = zone_size_byte;
      ...
      config_.shadow_config.set_zone_capacity = zone_size_byte / 4;
    }
    ...
    config_.shadow_config.total_zone_num = max_num_zones - num_clean_zones;
    config_.shadow_config.initial_set_zone_num = initial_num_zones - num_clean_zones;
  }
...
std::unique_ptr<Device> createZNSDevice(
  ...
  return createDirectIoZNSDevice(std::move(fileName), singleFileSize, nr_zones,
                                 ...
                                 maxDeviceWriteSize, char_device_path);
}
