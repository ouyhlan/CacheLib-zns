...
class ZoneHashConfig {
 public:
  ZoneHashConfig& setCharDevicePath(std::string path) noexcept {
    char_device_path_ = path;
    return *this;
  }
  ...
  ZoneHashConfig& setGarbageCollection(uint32_t log_num_clean_zones,
                                       uint32_t set_num_clean_zones) {
    log_num_clean_zones_ = log_num_clean_zones;
    set_num_clean_zones_ = set_num_clean_zones;
    return *this;
  }

  bool enable() const { return enable_; }

  std::string getCharDevicePath() const { return char_device_path_; }
  ...
  auto getNumCleanZones() const -> std::tuple<uint32_t, uint32_t> {
    return {log_num_clean_zones_, set_num_clean_zones_};
  }
 ...
 private:
  ...
  uint32_t log_num_clean_zones_{1};
  uint32_t set_num_clean_zones_{2};
  uint32_t set_admit_threshold_{1};
  uint32_t adaptive_pct_{0};
  std::string char_device_path_{};
};
